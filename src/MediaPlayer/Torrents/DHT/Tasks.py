...
from MediaPlayer.Torrents.DHT.Messages import QueryDHTMessage
from MediaPlayer.Torrents.DHT.Util import DHTTaskState

from MediaPlayer.Torrents.DHT.Node import Node
...
class PingTask(BaseTask):
    ...
    def ping_response(self, data):
        Logger().write(LogVerbosity.All, "DHT: Ping response received")
        self.complete()

    def ping_timeout(self):
        Logger().write(LogVerbosity.All, "DHT: Ping timed out")
        self.complete()
...
class FindNodeTask(BaseTask):
    ...
    def find_node_response(self, data):
        if b"nodes" in data.response:
            Logger().write(LogVerbosity.All, "DHT: FindNode request got a response with " + str(len(data.nodes) // 26) + " nodes")
            self.found_nodes += 1
    ...
    def find_node_timeout(self):
        Logger().write(LogVerbosity.All, "DHT: FindNode request timed out")
        self.check_done()

    def check_done(self):
        if self.outstanding_requests == 0:
            Logger().write(LogVerbosity.Debug, "DHT: FindNode found " + str(self.found_nodes) + " nodes")
            self.complete()
...
class GetPeersTask(BaseTask):
    ...
    def on_response(self, data):
        if b'values' in data.response:
            Logger().write(LogVerbosity.All, "DHT: GetPeers request got values response")
            ...
            Logger().write(LogVerbosity.All, "DHT: GetPeers request got nodes response")
    ...
    def on_timeout(self):
        Logger().write(LogVerbosity.All, "DHT: GetPeers request timed out")
        self.check_done()

    def check_done(self):
        if self.outstanding_requests == 0:
            Logger().write(LogVerbosity.Debug, "DHT: GetPeers found " + str(len(self.found_peers)) + " peers")
            self.complete()
