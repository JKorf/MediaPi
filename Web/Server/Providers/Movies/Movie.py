...
class Movie:
    ...
    @staticmethod
    def parse_list_from_yts(data):
        result = []
        json_data = json.loads(data)
        for movie_data in json_data['data']['movies']:
            result.append(Movie.parse_yts_movie(movie_data))
        return to_JSON(result)

    @staticmethod
    def parse_item_from_yts(data):
        return to_JSON(Movie.parse_yts_movie(json.loads(data)['data']['movie']))

    @staticmethod
    def parse_yts_movie(movie_data):
        movie = Movie()
        movie.id = movie_data['id']
        movie.imdb_id = movie_data['imdb_code']
        movie.url = movie_data['url']
        movie.title = movie_data['title']
        movie.year = movie_data['year']
        movie.rating_grade = movie_data['rating']
        movie.runtime = movie_data['runtime']
        movie.genres = movie_data['genres']
        movie.synopsis = movie_data['description_full']
        movie.youtube_trailer = "https://www.youtube.com/watch?v=" + movie_data['yt_trailer_code']
        movie.poster = movie_data['medium_cover_image']
        if 'date_uploaded' in movie_data:
            movie.released = movie_data['date_uploaded']
        movie.torrents = []
        for torrent_data in movie_data['torrents']:
            torrent = Torrent()
            torrent.url = torrent_data['url']
            torrent.seeds = torrent_data['seeds']
            torrent.peers = torrent_data['peers']
            torrent.quality = torrent_data['quality']
            torrent.size = torrent_data['size_bytes']
            movie.torrents.append(torrent)
        return movie